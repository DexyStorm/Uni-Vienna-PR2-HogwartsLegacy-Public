...
public abstract class Spell {
	...
	public Spell(String name, int manaCost, MagicLevel levelNeeded) 
	{
		if(name == null)
		{
			throw new IllegalArgumentException("name must not be null. error thrown from Spell class from ctor");
		}
		if(name.isEmpty())
		{
			throw new IllegalArgumentException("name must not be empty. error thrown from Spell class from ctor");
		}
		if(manaCost < 0)
		{
			throw new IllegalArgumentException("mana must no be negative. error thrown from Spell class from ctor");
		}
		if(levelNeeded == null)
		{
			throw new IllegalArgumentException("levelNeeded must not be null. error thrown from Spell class from ctor");
		}
		this.name = name;
	...
	{
		if(source.provideMana(levelNeeded, manaCost) == true)
		{
	...
	{
		return "[" + this.name + "(" + levelNeeded + "): " + this.manaCost + " mana" + additionalOutputString() + "]";
	}
