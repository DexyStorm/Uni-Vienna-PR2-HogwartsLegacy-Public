...
public class Main {
	...
	public static List<MagicItem> filterItems(List<MagicItem> items, int digit)	{
		List<MagicItem> array_to_return = new ArrayList<MagicItem>();
    ...
    {
        /*//geht nicht 4 some reason
        Potion[] array_to_return = new Potion[potions.size()];
        for(int i = 0; i < potions.size(); ++i)
        {
            array_to_return[i] = potions.at(i);
        }
        */
    	
    	//nur potions welche getweight < 20
    	
        //Potion[] array_to_return = potions.toArray(new Potion[0]); //puts the potions in a array
        ...
        Comparator<Potion> comp = Comparator.comparing(Potion::getName, Comparator.reverseOrder()).thenComparing(p->p.getWeight() + p.getPrice());
        
        Arrays.sort(array_to_return, comp);
	...
	public static Map<Integer, Set<Wizard>> mapWizardsPerNoSpells(List<Wizard> wizards) {
		Map<Integer, Set<Wizard>> map_to_return = new HashMap<Integer, Set<Wizard>>();
		
		for(Wizard w : wizards)
		{
			if(map_to_return.containsKey(w.getNumberOfKnownSpells()))
			{
				Set<Wizard> existing_set = map_to_return.get(w.getNumberOfKnownSpells());
				existing_set.add(w);
			}
			else
			{
				Set<Wizard> new_set = new HashSet<Wizard>();
				new_set.add(w);
				map_to_return.put(w.getNumberOfKnownSpells(), new_set);
			}
		}
